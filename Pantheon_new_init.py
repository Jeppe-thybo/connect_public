...
class Pantheon(Likelihood_sn):
    ...
    def __init__(self, path, data, command_line):
        ...
        # Changing the data to numpy arrays for faster evaluation in loglkl
        self.z_cmb = self.light_curve_params['zcmb'].to_numpy()
        self.z_hel = self.light_curve_params['zhel'].to_numpy()
        self.mb = self.light_curve_params['mb'].to_numpy()
        self.size = self.z_cmb.size
        ...
        self.cov_inv = la.inv(self.cov, overwrite_a=True)
    ...
    def loglkl(self, cosmo, data):
        ...
        #Attempt at changing previous block for faster evaluation
        size = self.size
        zcmb = self.z_cmb
        zhel = self.z_hel
        mb = self.mb

        moduli = (1 + zhel) / (1 + zcmb) * cosmo.luminosity_distance(zcmb)
        ...
        residuals = self.cov_inv @ residuals
